import Phaser from "phaser";

type BitCell = {
  row: number;
  col: number;
  x: number;
  y: number;
  value: number;
  targetValue: number;
  isWrong: boolean;
  rect: Phaser.GameObjects.Rectangle;
  text: Phaser.GameObjects.Text;
};

export default class Minigame3 extends Phaser.Scene {
  private bg?: Phaser.GameObjects.Image;

  private monitorX = 0;
  private monitorY = 0;
  private monitorW = 0;
  private monitorH = 0;

  private uiScale = 1;

  private rows = 4;
  private cols = 8;

  private gridStartX = 0;
  private gridStartY = 0;
  private cellW = 0;
  private cellH = 0;

  private cells: BitCell[] = [];
  private targetIndexes: number[] = [];

  private cursorRow = 0;
  private cursorCol = 0;
  private cursorRect?: Phaser.GameObjects.Rectangle;

  private progress = 0;
  private acceptingInput = false;

  private screenCenterY = 0;

  private progressFill?: Phaser.GameObjects.Rectangle;
  private progressText?: Phaser.GameObjects.Text;
  private statusText?: Phaser.GameObjects.Text;

  private grantedImage?: Phaser.GameObjects.Image;

  private titleText?: Phaser.GameObjects.Text;
  private subtitleText?: Phaser.GameObjects.Text;
  private progressBg?: Phaser.GameObjects.Rectangle;
  private panelBg?: Phaser.GameObjects.Rectangle;

  constructor() {
    super("Minigame3");
  }

  preload() {
    this.load.image("min3_computer", "../assets/images/min3/bg-laptop.png");
    this.load.image("min3_accessGranted", "../assets/images/min3/Access_granted.png");
  }

  create() {
    const { width, height } = this.scale;

    this.scale.off("resize");
    this.scale.on("resize", () => {
      this.scene.restart();
    });

    this.computeLayout(width, height);
    this.drawBackground();
    this.createCloseButton();
    this.createPanel();
    this.createTexts();
    this.createProgressBar();
    this.createBitGrid();
    this.createCursor();

    this.input.keyboard?.on("keydown", this.handleKeyPress, this);

    this.startRound();
  }

  private computeLayout(width: number, height: number) {
    this.monitorX = width / 2;
    this.monitorY = height / 2;

    this.monitorW = width * 0.34;
    this.monitorH = height * 0.33;

    this.uiScale = Math.min(width / 1366, height / 768);
    this.uiScale = Phaser.Math.Clamp(this.uiScale, 0.8, 1.35);

    this.screenCenterY = this.monitorY + this.ui(6);
  }

  private ui(v: number) {
    return v * this.uiScale;
  }

  private drawBackground() {
  const { width, height } = this.scale;

  this.bg = this.add.image(width / 2, height / 2 - 15, "min3_computer");
  this.bg.setOrigin(0.5);

  const tex = this.textures.get("min3_computer").getSourceImage() as HTMLImageElement;
  const texW = tex.width;
  const texH = tex.height;

  let scale = Math.max(width / texW, height / texH);

  scale *= 1.0; 

  this.bg.setScale(scale);
  this.bg.setDepth(0);
}

  private createCloseButton() {
    const closeX = this.scale.width * 0.723;
    const closeY = this.scale.height * 0.127;

    const closeText = this.add
      .text(closeX, closeY, "x", {
        fontFamily: "Pixelify Sans",
        fontSize: `${Math.max(24, Math.round(this.ui(24)))}px`,
        color: "#ffffff",
        fontStyle: "bold"
      })
      .setOrigin(0.5)
      .setDepth(20)
      .setInteractive({ useHandCursor: true });

    closeText.on("pointerover", () => closeText.setScale(1.12));
    closeText.on("pointerout", () => closeText.setScale(1));
    closeText.on("pointerdown", () => {
      this.input.keyboard?.off("keydown", this.handleKeyPress, this);
      this.scene.stop();
      this.scene.resume("GamePlay");
    });
  }

  private createPanel() {
    this.panelBg = this.add
      .rectangle(
        this.monitorX -6,
        this.monitorY - 79,
        this.monitorW *1.40,
        this.monitorH * 1.73,
        0x000000,
        0.96
      )
      .setOrigin(0.5)
      .setDepth(2);

    this.panelBg.setStrokeStyle(2, 0x0d0d0d, 0.35);
  }

  private createTexts() {
    const topY = this.screenCenterY - this.ui(230);

    this.titleText = this.add
      .text(this.monitorX, topY, "FILE MODIFICATION", {
        fontFamily: "Pixelify Sans",
        fontSize: `${Math.max(22, Math.round(this.ui(55)))}px`,
        color: "#46ff88",
        fontStyle: "bold",
        stroke: "#09100c",
        strokeThickness: Math.max(2, Math.round(this.ui(2)))
      })
      .setOrigin(0.5)
      .setDepth(5);

    this.subtitleText = this.add
      .text(this.monitorX, topY + this.ui(34), "MEMORIZE THE BITS, THEN PATCH THE FILE", {
        fontFamily: "Pixelify Sans",
        fontSize: `${Math.max(10, Math.round(this.ui(22)))}px`,
        color: "#8de8ff",
        fontStyle: "bold"
      })
      .setOrigin(0.5)
      .setDepth(5);

    this.statusText = this.add
      .text(this.monitorX, this.screenCenterY + this.ui(105), "", {
        fontFamily: "Pixelify Sans",
        fontSize: `${Math.max(11, Math.round(this.ui(20)))}px`,
        color: "#d7f8ff",
        fontStyle: "bold"
      })
      .setOrigin(0.5)
      .setAlpha(0)
      .setDepth(5);

    this.grantedImage = this.add
      .image(this.monitorX, this.monitorY + this.ui(8), "min3_accessGranted")
      .setOrigin(0.5)
      .setScale(this.uiScale * 0.78)
      .setAlpha(0)
      .setDepth(100);
  }

  private createProgressBar() {
    const barW = this.monitorW * 0.78;
    const barH = this.ui(35);
    const barY = this.screenCenterY - this.ui(120);

    this.progressBg = this.add
      .rectangle(this.monitorX, barY, barW, barH, 0x08111d, 0.96)
      .setStrokeStyle(2, 0x46ff88, 0.9)
      .setOrigin(0.5)
      .setDepth(5);

    this.progressFill = this.add
      .rectangle(
        this.monitorX - barW / 2 + this.ui(3),
        barY,
        0,
        barH - this.ui(4),
        0x44ff88,
        0.95
      )
      .setOrigin(0, 0.5)
      .setDepth(6);

    this.progressText = this.add
      .text(this.monitorX, barY, "0%", {
        fontFamily: "Pixelify Sans",
        fontSize: `${Math.max(10, Math.round(this.ui(20)))}px`,
        color: "#ffffff",
        fontStyle: "bold"
      })
      .setOrigin(0.5)
      .setDepth(7);
  }

  private createBitGrid() {
    const gridW = this.monitorW * 1.20;
    const gridH = this.monitorH * 0.66;

    this.cellW = gridW / this.cols;
    this.cellH = gridH / this.rows;

    this.gridStartX = this.monitorX - gridW / 2 + this.cellW / 2;
    this.gridStartY = this.screenCenterY + this.ui(5);

    this.cells = [];

    for (let r = 0; r < this.rows; r++) {
      for (let c = 0; c < this.cols; c++) {
        const x = this.gridStartX + c * this.cellW;
        const y = this.gridStartY - gridH / 2 + this.cellH / 2 + r * this.cellH;

        const value = Phaser.Math.Between(0, 1);

        const rect = this.add
          .rectangle(
            x,
            y,
            this.cellW - this.ui(6),
            this.cellH - this.ui(6),
            0x0d1723,
            0.94
          )
          .setStrokeStyle(2, 0x36ddff, 0.18)
          .setInteractive({ useHandCursor: true })
          .setDepth(5);

        const text = this.add
          .text(x, y, String(value), {
            fontFamily: "Pixelify Sans",
            fontSize: `${Math.max(11, Math.round(this.ui(19)))}px`,
            color: value === 1 ? "#46ff88" : "#e0f9ff",
            fontStyle: "bold"
          })
          .setOrigin(0.5)
          .setDepth(6);

        const cell: BitCell = {
        row: r,
        col: c,
        x,
        y,
        value,
        targetValue: value,
        isWrong: false,
        rect,
        text
      };

        rect.on("pointerdown", () => {
          if (!this.acceptingInput) return;
          this.cursorRow = r;
          this.cursorCol = c;
          this.updateCursorPosition();
          this.toggleCurrentCell();
        });

        this.cells.push(cell);
      }
    }
  }

  private createCursor() {
    this.cursorRect = this.add
      .rectangle(0, 0, this.cellW - this.ui(3), this.cellH - this.ui(3))
      .setStrokeStyle(2, 0xffd84a, 1)
      .setFillStyle(0xffffff, 0)
      .setOrigin(0.5)
      .setDepth(7);

    this.updateCursorPosition();
  }

  private startRound() {
    this.acceptingInput = false;
    this.progress = 0;
    this.updateProgressUI();

    this.generateTargets();
    this.updateGridVisuals(true);
    this.showStatus(" ", "#8de8ff");

    this.time.delayedCall(3200, () => {
      this.acceptingInput = true;
      this.updateGridVisuals(false);
      this.showStatus("EDIT THE FILE", "#46ff88");
    });
  }

  private generateTargets() {
    this.targetIndexes = [];

    for (const cell of this.cells) {
      cell.targetValue = cell.value;
    }

    const count = 5;
    const allIndexes = Phaser.Utils.Array.NumberArray(0, this.cells.length - 1);
    Phaser.Utils.Array.Shuffle(allIndexes);

    this.targetIndexes = allIndexes.slice(0, count);

    for (const index of this.targetIndexes) {
      const cell = this.cells[index];
      cell.targetValue = cell.value === 0 ? 1 : 0;
    }
  }

  private handleKeyPress(event: KeyboardEvent) {
    if (event.code === "ArrowUp") {
      if (!this.acceptingInput) return;
      this.cursorRow = (this.cursorRow - 1 + this.rows) % this.rows;
      this.updateCursorPosition();
      return;
    }

    if (event.code === "ArrowDown") {
      if (!this.acceptingInput) return;
      this.cursorRow = (this.cursorRow + 1) % this.rows;
      this.updateCursorPosition();
      return;
    }

    if (event.code === "ArrowLeft") {
      if (!this.acceptingInput) return;
      this.cursorCol = (this.cursorCol - 1 + this.cols) % this.cols;
      this.updateCursorPosition();
      return;
    }

    if (event.code === "ArrowRight") {
      if (!this.acceptingInput) return;
      this.cursorCol = (this.cursorCol + 1) % this.cols;
      this.updateCursorPosition();
      return;
    }

    if (event.code === "Space" || event.code === "Enter" || event.code === "NumpadEnter") {
      if (!this.acceptingInput) return;
      this.toggleCurrentCell();
    }
  }

  private toggleCurrentCell() {
  const cell = this.getCell(this.cursorRow, this.cursorCol);
  if (!cell) return;

  cell.value = cell.value === 0 ? 1 : 0;
  cell.text.setText(String(cell.value));

  this.flashCell(cell);

  if (cell.value === cell.targetValue) {
    cell.isWrong = false;
    this.showStatus("BIT MODIFIED", "#46ff88");
  } else {
    cell.isWrong = true;
    this.showStatus("ERROR: WRONG BIT", "#ff4d6d");
    this.errorCell(cell);
  }

  this.updateGridVisuals(false);
  this.updateProgressFromState();

  if (this.isTaskCompleted()) {
    this.completeTask();
  }
}

  private getCell(row: number, col: number) {
    return this.cells.find((cell) => cell.row === row && cell.col === col);
  }

  private updateCursorPosition() {
    if (!this.cursorRect) return;

    const cell = this.getCell(this.cursorRow, this.cursorCol);
    if (!cell) return;

    this.cursorRect.setPosition(cell.x, cell.y);
  }

  private updateGridVisuals(showTargets: boolean) {
  for (let i = 0; i < this.cells.length; i++) {
    const cell = this.cells[i];
    const isCursor = cell.row === this.cursorRow && cell.col === this.cursorCol;
    const isTarget = this.targetIndexes.includes(i);

    let fill = 0x0d1723;
    let stroke = 0x36ddff;
    let strokeAlpha = 0.22;
    let textColor = cell.value === 1 ? "#46ff88" : "#e0f9ff";

    if (showTargets && isTarget) {
      fill = 0x2a1030;
      stroke = 0xff57c7;
      strokeAlpha = 1;
    } else if (!showTargets && cell.isWrong) {
      fill = 0x3a1010;
      stroke = 0xff4d6d;
      strokeAlpha = 1;
      textColor = "#ff8aa0";
    } else if (!showTargets && cell.value === cell.targetValue && isTarget) {
      fill = 0x12291d;
      stroke = 0x46ff88;
      strokeAlpha = 0.75;
    }

    if (isCursor) {
      stroke = 0xffd84a;
      strokeAlpha = 1;
    }

    cell.rect.setFillStyle(fill, 0.97);
    cell.rect.setStrokeStyle(2, stroke, strokeAlpha);
    cell.text.setColor(textColor);
  }
}

  private updateProgressFromState() {
    let correctTargets = 0;
    let wrongBits = 0;

    for (let i = 0; i < this.cells.length; i++) {
      const cell = this.cells[i];
      const isTarget = this.targetIndexes.includes(i);

      if (isTarget && cell.value === cell.targetValue) {
        correctTargets++;
      }

      if (!isTarget && cell.value !== cell.targetValue) {
        wrongBits++;
      }
    }

    const step = 100 / this.targetIndexes.length;

    this.progress = Math.max(
      0,
      Math.min(100, Math.round(correctTargets * step - wrongBits * step))
    );

    this.updateProgressUI();
  }

  private updateProgressUI() {
    if (!this.progressFill || !this.progressText) return;

    const maxWidth = this.monitorW * 0.58 - this.ui(6);
    const targetWidth = (this.progress / 100) * maxWidth;

    this.tweens.add({
      targets: this.progressFill,
      width: targetWidth,
      duration: 180,
      ease: "Sine.Out"
    });

    this.progressText.setText(`${Math.round(this.progress)}%`);
  }

  private flashCell(cell: BitCell) {
    this.tweens.add({
      targets: cell.text,
      scaleX: 1.16,
      scaleY: 1.16,
      duration: 90,
      yoyo: true
    });
  }

  private errorCell(cell: BitCell) {
  this.tweens.add({
    targets: [cell.rect, cell.text],
    x: "+=6",
    duration: 35,
    yoyo: true,
    repeat: 3,
    onComplete: () => {
      this.updateGridVisuals(false);
    }
  });
}

  private showStatus(message: string, color: string) {
    if (!this.statusText) return;

    this.tweens.killTweensOf(this.statusText);

    this.statusText.setText(message);
    this.statusText.setColor(color);
    this.statusText.setAlpha(1);
    this.statusText.setScale(0.92);

    this.tweens.add({
      targets: this.statusText,
      scaleX: 1,
      scaleY: 1,
      duration: 100
    });

    this.tweens.add({
      targets: this.statusText,
      alpha: 0,
      duration: 350,
      delay: 700
    });
  }

  private isTaskCompleted() {
    for (const cell of this.cells) {
      if (cell.value !== cell.targetValue) return false;
    }
    return true;
  }

  private hideMonitorUI() {
    this.titleText?.setVisible(false);
    this.subtitleText?.setVisible(false);
    this.statusText?.setVisible(false);

    this.progressBg?.setVisible(false);
    this.progressFill?.setVisible(false);
    this.progressText?.setVisible(false);

    this.cursorRect?.setVisible(false);

    for (const cell of this.cells) {
      cell.rect.setVisible(false);
      cell.text.setVisible(false);
    }
  }

  private completeTask() {
    this.acceptingInput = false;
    this.input.keyboard?.off("keydown", this.handleKeyPress, this);

    this.progress = 100;
    this.updateProgressUI();
    this.registry.set("task3Completed", true);

    this.hideMonitorUI();

    if (this.panelBg) {
      this.panelBg.setFillStyle(0x000000, 1);
      this.panelBg.setDepth(90);
    }

    if (this.grantedImage) {
      this.grantedImage.setPosition(this.monitorX, this.monitorY + this.ui(-88));
      this.grantedImage.setAlpha(1);
      this.grantedImage.setScale(this.uiScale * 2.38);
      this.grantedImage.setDepth(100);
    }

    this.time.delayedCall(2200, () => {
      this.scene.stop();
      this.scene.resume("GamePlay");
    });
  }
}