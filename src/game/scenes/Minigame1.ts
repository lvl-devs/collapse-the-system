...
export default class Minigame1 extends Phaser.Scene {
  ...
  private createSocketsAndWires() {
    ...
    const bottomY = this.screenY + this.screenH / 2 - this.screenH * 0.20;
    ...
    const plan = this.buildSocketPlan(slots, allSidePoints);
    const leftSlots = plan.leftSlots;
    const rightSlots = plan.rightSlots;
    const topSlots = plan.topSlots;
    const bottomSlots = plan.bottomSlots;
  ...
  private createWire(color: WireColor, socketA: SocketData, socketB: SocketData) {
    ...
    plug.add(plugGfx);
    this.drawPlugGraphic(plugGfx, color.hex, socketA.side, true, false);
  ...
  private tryConnect(wire: WireState) {
    ...
    if (dist < snapRadius && selfDist > 10) {
      ...
      this.drawSocketGraphic(target.gfx, target.color, target.side, true, false);
      this.redrawCable(wire);
